---
title: Figure 2 (D-E), Measurement of boundary distances
...
source(here("figure_2_d-e/", "helpers", "plot_functions.R"))
```
...
Set `glob` patterns for directory searches for cell level data and spot data,resepectively. The spot level data is stored in a single .csv file per well where all the spots in all the channels are stored. The `channel` variable value indicates the channel in which the spot was detected according to this mapping: - 2: Green (488) - 3: Red (561) - 4: Far Red (640)
...
Cell filtering based on spots number Keep only cells that have 2 spots in the Green channel, 2 spots in the Red channel, and 2 or less spots in the FarRed channel.
...
Classify looping status Establish that: - green-red (3'-5') spots who are equal to or smaller than 2.5 microns in 2D are looped - green-red (3'-5') spots who are larger than 2.5 microns in 2D are unlooped
...
# Check the groups
group1 <- gfr_dist_min_2D$gr_dist[
  gfr_dist_min_2D$treat == unique(gfr_dist_min_2D$treat)[1]
]
group2 <- gfr_dist_min_2D$gr_dist[
  gfr_dist_min_2D$treat == unique(gfr_dist_min_2D$treat)[2]
]
